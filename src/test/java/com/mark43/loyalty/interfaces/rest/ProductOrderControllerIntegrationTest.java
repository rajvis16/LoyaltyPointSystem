...
class ProductOrderControllerIntegrationTest {
    ...
    public void verifyPointClawbackWithPriorRedemptionDeficit() throws Exception {
        ...
        // 1. Primary Purchase: $600.00 Spend -> 600.00 points
        mockMvc.perform(post("/api/v1/orders/buy").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new OrderRequestDTO(email, "TXN-TEST-001", Map.of("Premium Coffee Maker", 3))))).andExpect(status().isCreated());

        // 2. Secondary Purchase: $200.00 Spend -> +200.00 points (Total: 800)
        mockMvc.perform(post("/api/v1/orders/buy").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new OrderRequestDTO(email, "TXN-TEST-002", Map.of("Premium Coffee Maker", 1))))).andExpect(status().isCreated());

        // 3. FIFO Redemption: Consumes 100.00 points from the oldest open bucket (Total: 700)
        mockMvc.perform(post("/api/v1/loyalty/redeem").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new RedeemRewardDTO(email, "Branded Travel Mug")))).andExpect(status().isOk());

        // 4. Full Return of First Order: Clawback requested: 600.00 points
        mockMvc.perform(post("/api/v1/orders/return").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new OrderRequestDTO(email, "TXN-TEST-001", Map.of("Premium Coffee Maker", 3))))).andExpect(status().isOk());

        // 5. ASSERTION: Relational balance math must hold up under stress
        // Formula: 600 (Earn 1) + 200 (Earn 2) - 100 (Redeem) - 600 (Clawback) = 100.00 pts
        mockMvc.perform(get("/api/v1/loyalty/balance/email").param("email", email))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.rollingSpend").value(200.00))

                // 💥 FAIL-POINT: Current SUM(remainingPoints) bug will return 200.00 instead of 100.00
                .andExpect(jsonPath("$.pointsBalance").value(100.00));
    }
}